/*
  ...
  1. New Tables
    - `product_types` - Define os tipos de produtos disponíveis
    - `product_variations` - Armazena as variações dos produtos
    - `product_variation_options` - Armazena as opções de cada variação
    - `product_components` - Armazena os componentes de kits e produtos fabricados
    - `product_attributes` - Armazena os atributos dos produtos

  2. Changes
    - Adiciona coluna `type` na tabela `products`
    - Adiciona coluna `parent_id` na tabela `products` para variações
    - Adiciona coluna `attributes` na tabela `products`

  3. Security
    - Enable RLS em todas as novas tabelas
    - Adiciona políticas de acesso baseadas na loja
*/
